(function($){
	...
	fields.field_group = fields.model.extend({
		...
		*/
		...
		init: function(){
			...
			// $el
			this.$fields = $('#fields-field-group-fields');
			this.$locations = $('#fields-field-group-locations');
			this.$options = $('#fields-field-group-options');
			
			
			// disable validation
			fields.validation.active = 0;
			
			
			// sort fields
			this.sort_fields( $('.fields-field-list:first') );
		...
		*/
		...
		get_fields: function( $fields ) {
			...
			return $fields.not('.fields-field-object[data-id="fieldscloneindex"] .fields-field');
		...
		*/
		...
		render_fields: function(){
			...
			$('.fields-field-list').each(function(){
				
				// vars
				var $fields = $(this).children('.fields-field-object').not('[data-id="fieldscloneindex"]');
				...
				$fields.each(function( i ){
					...
					// update icon number
					$(this).children('.handle').find('.fields-icon').html( i+1 );
		...
		*/
		...
		update_field_meta: function( $el, name, value ){
			...
			if( !$input.exists() ) {
				...
				// replcae
				html = fields.str_replace('ID', name, html);
		...
		*/
		...
		save_field: function( $el, type ){
			...
			fields.do_action('save_field', $el, type);
		...
		*/
		...
		submit: function( e ){
			...
			if( !$title.val() ) {
				...
				// unlock form
				fields.validation.toggle( e.$el, 'unlock' );
				
				
				// alert
				alert( fields._e('title_is_required') );
			...
			// close / delete fields
			$('.fields-field-object').each(function(){
		...
		*/
		...
		trash: function( e ){
			...
			var result = confirm( fields._e('move_to_trash') );
		...
		*/
		...
		render_field: function( $el ){
			...
			// update required
			$el.find('> .handle .li-field-label .fields-required').remove();
			
			if( required == '1' ) {
				
				$el.find('> .handle .li-field-label strong').append('<span class="fields-required">*</span>');
		...
		*/
		...
		open_field: function( $el ){
			...
			// action for 3rd party customization
			fields.do_action('open_field', $el);
		...
		*/
		...
		close_field: function( $el ){
			...
			// action for 3rd party customization
			fields.do_action('close_field', $el);
		...
		*/
		...
		wipe_field: function( $el ){
			...
			// action for 3rd party customization
			fields.do_action('wipe_field', $el);
		...
		*/
		...
		add_field: function( $fields ){
			...
			// clone tr
			var $clone = $fields.children('.fields-field-object[data-id="fieldscloneindex"]'),
				$el = $clone.clone(),
		...
		*/
		...
		duplicate_field: function( $el ){
			...
			$label.val( $label.val() + ' (' + fields._e('copy') + ')' );
			$name.val( $name.val() + '_' + fields._e('copy') );
			...
			// action for 3rd party customization
			fields.do_action('duplicate_field', $el2);
		...
		*/
		...
		move_field: function( $field ){
			...
			var ajax_data = fields.prepare_for_ajax({
				'action':	'fields/field_group/move_field',
			...
			if( !ajax_data.field_id ) {
				...
				$field.find('.fields-field-object').not('[data-id="fieldscloneindex"]').each(function(){
			...
			if( warning ) {
				...
				alert( fields._e('move_field_warning') );
				return;
		...
		*/
		...
		move_field_confirm: function( $field, html ){
			...
			// submit form
			$('#fields-move-field-form').on('submit', function(){
		...
		*/
		...
		delete_field: function( $el, animation ){
			...
			// action for 3rd party customization
			fields.do_action('delete_field', $el);
		...
		*/
		...
		remove_field: function( $el ){
			...
			// vars
			var $field_list	= $el.closest('.fields-field-list');
			...
			if( $field_list.children('.fields-field-object').length == 1 ) {
		...
		*/
		...
		change_field_type: function( $select ){
			...
			var $tbody		= $select.closest('tbody'),
				$el			= $tbody.closest('.fields-field-object'),
				$parent		= $el.parent().closest('.fields-field-object'),
			...
			// update class
			$el.removeClass('fields-field-object-' + old_type.replace('_', '-'));
			$el.addClass('fields-field-object-' + new_type.replace('_', '-'));
			...
			// get settings
			var $settings = $tbody.children('.fields-field[data-setting="' + old_type + '"]'),
				html = '';
			...
			fields.update( key + '_settings_' + old_type, html );
			...
			html = fields.get( key + '_settings_' + new_type );
			
			if( html ) {
				
				// append settings
				$tbody.children('.fields-field[data-name="conditional_logic"]').before( html );
				
				
				// remove field settings html
				fields.update( key + '_settings_' + new_type, '' );
				
				
				// trigger event
				fields.do_action('change_field_type', $el);
			...
			// add loading
			var $tr = $('<tr class="fields-field"><td class="fields-label"></td><td class="fields-input"><div class="fields-loading"></div></td></tr>');
			
			
			// add $tr
			$tbody.children('.fields-field[data-name="conditional_logic"]').before( $tr );
			
			
			var ajax_data = {
				action		: 'fields/field_group/render_field_settings',
				nonce		: fields.o.nonce,
				parent		: fields.o.post_id,
				field_group	: fields.o.post_id,
			...
			var xhr = $.ajax({
				...
				success: function( html ){
					...
					// trigger event
					fields.do_action('append', $new_tr);
					fields.do_action('change_field_type', $el);
		...
		*/
		...
		change_field_label: function( $el ) {
			...
			// action for 3rd party customization
			fields.do_action('change_field_label', $el);
		...
		*/
		...
		change_field_name: function( $el ) {
			...
			if( $name.val().substr(0, 6) === 'field_' ) {
				...
				alert( fields._e('field_name_start') );
	...
	*/
	...
	fields.field_group.field = fields.model.extend({
		...
		events: {
			...
			'change .fields-field-object input':		'save',
			'change .fields-field-object textarea':	'save',
			'change .fields-field-object select':		'save'
		},
		
		event: function( e ){
			
			// append $field
			e.$field = e.$el.closest('.fields-field-object');
		...
		*/
		...
		edit: function( e ){
			...
			fields.field_group.edit_field( e.$field );
				
		},
		
		duplicate: function( e ){
			
			fields.field_group.duplicate_field( e.$field );
				
		},
		
		move: function( e ){
			
			fields.field_group.move_field( e.$field );
				
		},
		
		delete: function( e ){
			
			fields.field_group.delete_field( e.$field );
				
		},
		
		add: function( e ){
			
			var $list = e.$el.closest('.fields-field-list-wrap').children('.fields-field-list');
			
			fields.field_group.add_field( $list );
				
		},
		
		change_type: function( e ){
			
			fields.field_group.change_field_type( e.$el );
			
		},
		
		change_label: function( e ){
			
			fields.field_group.change_field_label( e.$field );
			
		},
		
		change_name: function( e ){
			
			fields.field_group.change_field_name( e.$field );
			
		},
		
		render: function( e ){
			
			fields.field_group.render_field( e.$field );
				
		},
		
		save: function( e ){
			
			fields.field_group.save_field( e.$field );
	...
	*/
	...
	fields.field_group.conditional_logic = fields.model.extend({
		...
		*/
		...
		render_fields: function(){
			...
			$('.fields-field-object.open').each(function(){
		...
		*/
		...
		render_field: function( $field ){
			...
			var key			= $field.attr('data-key'),
				$ancestors	= $field.parents('.fields-field-list'),
				$tr			= $field.find('.fields-field[data-name="conditional_logic"]:last');
			...
			$.each( $ancestors, function( i ){
				...
				// vars
				var group = (i == 0) ? fields._e('sibling_fields') : fields._e('parent_fields');
				
				
				// loop over fields
				$(this).children('.fields-field-object').each(function(){
					...
					} else if( this_key == 'fieldscloneindex' ) {
		...
		*/
		...
		render_rule: function( $tr, triggers ) {
			...
			if( triggers ) {
				...
				fields.render_select( $trigger, triggers );
				
			}
			
			
			// vars
			var $field		= $('.fields-field-object[data-key="' + $trigger.val() + '"]'),
			...
			// select				
				...
				// vars
				var lines = $field.find('.fields-field[data-name="choices"] textarea').val().split("\n");	
				...
				// allow null
				var $allow_null = $field.find('.fields-field[data-name="allow_null"]');
			...
			// update select
			fields.render_select( $value, choices );
		...
		*/
		...
		add_group: function( e ){
			...
			// duplicate
			$group2 = fields.duplicate( $group );
			
			
			// update h4
			$group2.find('h4').text( fields._e('or') );
	...
	*/
	...
	fields.field_group.locations = fields.model.extend({
		...
		*/
		...
		change_rule: function( e ){
			...
			// add loading gif
			var $div = $('<div class="fields-loading"></div>');
			...
			$.ajax({
				...
				data: fields.prepare_for_ajax({
					'action':	'fields/field_group/render_location_value',
					'rule_id':	rule_id,
					'group_id':	group_id,
					'param':	$select.val(),
					'value':	''
	...
	*/
	...
	fields.add_action('open_field change_field_type', function( $el ){
		
		// clear name
		$el.find('.fields-field[data-append]').each(function(){
			...
			// vars
			var $wrap = $sibling.children('.fields-input'),
				$ul = $wrap.children('.fields-hl');
			
			
			if( !$ul.exists() ) {
				
				$wrap.wrapInner('<ul class="fields-hl"><li></li></ul>');
			...
			// create $li
			var $li = $('<li></li>').append( $(this).children('.fields-input').children() );
	...
	*/
	...
	var fields_settings_select = fields.model.extend({
		...
		events: {
			'change .fields-field[data-name="ui"] input': 'render'
		},
		...
		render: function( $el ){
			...
			// vars
			var val = $el.find('.fields-field[data-name="ui"] input:checked').val();
			
			
			// show / hide
			if( val == '1' ) {
			
				$el.find('.fields-field[data-name="ajax"]').show();
				
			} else {
			
				$el.find('.fields-field[data-name="ajax"]').hide();
	...
	*/
	...
	var fields_settings_radio = fields.model.extend({
		...
		events: {
			'change .fields-field[data-name="other_choice"] input': 'render'
		},
		...
		render: function( $el ){
			...
			// vars
			var val = $el.find('.fields-field[data-name="other_choice"] input:checked').val();
			
			if( val == '1' ) {
				
				$el.find('.fields-field[data-name="save_other_choice"]').show();
				
			} else {
				
				$el.find('.fields-field[data-name="save_other_choice"]').hide();
				$el.find('.fields-field[data-name="save_other_choice"] input').prop('checked', false);
	...
	*/
	...
	var fields_settings_date_picker = fields.model.extend({
		...
		events: {
			'change .fields-field[data-name="display_format"] input':	'render',
			'change .fields-field[data-name="return_format"] input':	'render'
		},
		...
		render: function( $el ){
			...
			$.each(['display_format', 'return_format'], function(k,v){
				...
				// vars
				var $radio = $el.find('.fields-field[data-name="' + v + '"] input[type="radio"]:checked'),
					$other = $el.find('.fields-field[data-name="' + v + '"] input[type="text"]');
	...
	*/
	...
	var fields_settings_tab = fields.model.extend({
		...
		render: function( $el ){
			...
			// clear name
			$el.find('.fields-field[data-name="name"] input').val('').trigger('change');
	...
	*/
	...
	var fields_settings_message = fields.model.extend({
	...
	*/
	...
	fields.field_group.screen = fields.model.extend({
		...
		events: {
			'click #fields-field-key-hide': 'toggle'
		},
		...
		ready: function(){
			...
			// vars
			var $el = $('#adv-settings'),
				$append = $el.find('#fields-append-show-on-screen');
		...
		toggle: function( e ){
			...
			// update user setting
			fields.update_user_setting('show_field_keys', val);
		...
		render: function(){
			...
			// vars
			var options = fields.serialize_form( $('#adv-settings') );
			...
			// toggle class
			var $fields = fields.field_group.$fields;
