(function($){
	...
	// comon
	fields.pro = fields.model.extend({
		...
		get_fields : function( $fields ){
			...
			// remove clone fields
			$fields = $fields.not('.fields-clone .fields-field');
		...
		*  refresh
		*
		*  This function will run when fields detects a refresh is needed on the UI
		...
		*/
		...
		refresh: function(){
			...
			$('.fields-table').each(function(){
				
				// vars
				var $table = $(this),
					$ths = $table.find('> thead th.fields-th');
		...
		render_table : function( $table ){
			...
			// vars
			var $th = $table.find('> thead th.fields-th'),
				available_width = 100;
...
(function($){
	...
	fields.fields.repeater = fields.field.extend({
		...
		events: {
			'click .fields-repeater-add-row': 		'add',
			'click .fields-repeater-remove-row': 	'remove',
		},
		
		focus: function(){
			
			this.$el = this.$field.find('.fields-repeater:first');
			this.$input = this.$field.find('input:first');
			this.$tbody = this.$el.find('tbody:first');
			this.$clone = this.$tbody.children('tr.fields-clone');
		...
		initialize: function(){
			...
			// CSS fix
			this.$tbody.on('mouseenter', 'tr.fields-row', function( e ){
				
				// vars
				var $tr = $(this),
					$td = $tr.children('.remove'),
					$a = $td.find('.fields-repeater-add-row'),
			...
			// set column widths
			// no longer needed due to refresh action in fields.pro model
			//fields.pro.render_table( this.$el.children('table') );
		...
		show: function(){
			...
			this.$tbody.find('.fields-field:visible').each(function(){
		...
		render: function(){
			...
			if( this.o.max > 0 && this.count() >= this.o.max ) {
				...
				this.$el.find('> .fields-hl .fields-button').addClass('disabled');
				
			} else {
				
				this.$el.find('> .fields-hl .fields-button').removeClass('disabled');
		...
		add: function( e ){
			...
			if( e && e.$el.is('.fields-icon') ) {
			
				$before	= e.$el.closest('.fields-row');
			...
			if( this.o.max > 0 && this.count() >= this.o.max ) {
				...
				alert( fields._e('repeater','max').replace('{max}', this.o.max) );
				return false;
			...
			var new_id = fields.get_uniqid(),
				html = this.$clone.outerHTML();
				
				
			// replace fieldscloneindex
			var html = html.replace(/(="[\w-\[\]]+?)(fieldscloneindex)/g, '$1' + new_id),
				$html = $( html );
			
			
			// remove clone class
			$html.removeClass('fields-clone');
			
			
			// enable inputs (ignore inputs disabled for life)
			$html.find('input, textarea, select').not('.fields-disabled').removeAttr('disabled');
			...
			this.$field.parents('.fields-row').trigger('mouseenter');
		...
		remove: function( e ){
			...
			// vars
			var $tr = e.$el.closest('.fields-row'),
			...
			if( this.count() <= this.o.min ) {
				...
				alert( fields._e('repeater','min').replace('{min}', this.o.min) );
				return false;
			}
			
			
			// action for 3rd party customization
			fields.do_action('remove', $tr);
			
			
			// animate out tr
			fields.remove_tr( $tr, function(){
				...
				// trigger mouseenter on parent repeater to work out css margin on add-row button
				$field.closest('.fields-row').trigger('mouseenter');
				...
				if( $table.hasClass('table-layout') ) {
					...
					fields.conditional_logic.render( $table );
...
(function($){
	...
	fields.fields.flexible_content = fields.field.extend({
		...
		events: {
			'click .fields-fc-remove': 		'remove',
			'click .fields-fc-layout-handle':	'toggle',
			'click .fields-fc-toggle':			'toggle',
			'click .fields-fc-popup li a':		'add',
			'click .fields-fc-add': 			'open_popup',
			'blur .fields-fc-popup .focus':	'close_popup'
		},
		
		focus: function(){
			
			this.$el = this.$field.find('.fields-flexible-content:first');
			this.$input = this.$field.find('input:first');
		...
		initialize: function(){
			...
			if( this.o.max != 1 ) {
				...
				$values.one('mouseenter', '.fields-fc-layout-handle', function( e ){
					
					$values.unbind('sortable').sortable({
					
						items					: '> .layout',
						handle					: '> .fields-fc-layout-handle',
						forceHelperSize			: true,
						forcePlaceholderSize	: true,
						scroll					: true,
		...
		show: function(){
			...
			this.$values.find('.fields-field:visible').each(function(){
		...
		render: function(){
			...
			this.$values.children('.layout').each(function( i ){
				...
				$(this).find('> .fields-fc-layout-handle .fc-layout-order').html( i+1 );
		...
		validate_add : function( layout ){
			...
			if( this.o.max > 0 && this.count() >= this.o.max ) {
				...
				var identifier	= ( this.o.max == 1 ) ? 'layout' : 'layouts',
					s 			= fields._e('flexible_content', 'max');
				
				
				// translate
				s = s.replace('{max}', this.o.max);
				s = s.replace('{identifier}', fields._e('flexible_content', identifier));
			...
			if( layout_max > 0 && layout_count >= layout_max ) {
				...
				var identifier	= ( layout_max == 1 ) ? 'layout' : 'layouts',
					s 			= fields._e('flexible_content', 'max_layout');
		...
		validate_remove : function( layout ){
			...
			if( this.o.min > 0 && this.count() <= this.o.min ) {
				...
				var identifier	= ( this.o.min == 1 ) ? 'layout' : 'layouts',
					s 			= fields._e('flexible_content', 'min') + ', ' + fields._e('flexible_content', 'remove');
				
				
				// translate
				s = s.replace('{min}', this.o.min);
				s = s.replace('{identifier}', fields._e('flexible_content', identifier));
				s = s.replace('{layout}', fields._e('flexible_content', 'layout'));
			...
			if( layout_min > 0 && layout_count <= layout_min ) {
				...
				var identifier	= ( layout_min == 1 ) ? 'layout' : 'layouts',
					s 			= fields._e('flexible_content', 'min_layout') + ', ' + fields._e('flexible_content', 'remove');
			...
			$popup.find('a').each(function(){
				...
				if( max > 0 ) {
					...
					var available	= max - count,
						s			= fields._e('flexible_content', 'available'),
				...
				if( min > 0 ) {
					...
					var required	= min - count,
						s			= fields._e('flexible_content', 'required'),
		...
		add : function( e ){
			...
			// vars
			var $popup = e.$el.closest('.fields-fc-popup'),
		...
		remove: function( e ){
			...
			// action for 3rd party customization
			fields.do_action('remove', $layout);
			
			
			// remove
			fields.remove_el( $layout, function(){
		...
		toggle : function( e ){
			...
			if( $layout.hasClass('closed') ) {
				...
				// refresh layout
				fields.do_action('refresh', $layout);
		...
		sync : function(){
			...
			// vars
			var name = 'fields_collapsed_' + fields.get_data(this.$field, 'key'),
				collapsed = [];
			...
			fields.update_cookie( name, collapsed.join('|') );	
...
(function($){
	...
	fields.fields.gallery = fields.field.extend({
		...
		events: {
			'click .fields-gallery-attachment': 		'select_attachment',
			...
			'change .fields-gallery-side input':		'update_attachment',
			'change .fields-gallery-side textarea':	'update_attachment',
			'change .fields-gallery-side select':		'update_attachment',
			'change .bulk-actions':					'sort'
		},
		
		focus: function(){
			
			this.$el = this.$field.find('.fields-gallery').first();
			this.$values = this.$el.children('.values');
			this.$clones = this.$el.children('.clones');
		...
		get_attachment: function( id ){
			...
			// vars
			var selector = '.fields-gallery-attachment';
		...
		initialize: function(){
			...
			// sortable
			this.$el.find('.fields-gallery-attachments').unbind('sortable').sortable({
				
				items					: '.fields-gallery-attachment',
				forceHelperSize			: true,
				forcePlaceholderSize	: true,
				scroll					: true,
			...
			this.$el.unbind('resizable').resizable({
				...
				stop: function(event, ui){
					...
					fields.update_user_setting('gallery_height', ui.size.height);
		...
		sort: function( e ){
			...
			var data = fields.prepare_for_ajax({
				action		: 'fields/fields/gallery/get_sort_order',
				field_key	: fields.get_field_key(this.$field),
		...
		sort_success: function( json ) {
			...
			for( i in json.data ) {
				...
				// prepend attachment
				this.$el.find('.fields-gallery-attachments').prepend( $attachment );
		...
		open_sidebar: function(){
			...
			// animate
			this.$el.find('.fields-gallery-side-inner').css({ 'width' : width-1 });
			this.$el.find('.fields-gallery-side').animate({ 'width' : width-1 }, 250);
			this.$el.find('.fields-gallery-main').animate({ 'right' : width }, 250);
		...
		close_sidebar: function(){
			...
			// disable sidebar
			this.$el.find('.fields-gallery-side').find('input, textarea, select').attr('disabled', 'disabled');
			
			
			// animate
			this.$el.find('.fields-gallery-main').animate({ right: 0 }, 250);
			this.$el.find('.fields-gallery-side').animate({ width: 0 }, 250, function(){
				
				$select.show();
				
				$(this).find('.fields-gallery-side-data').html( '' );
		...
		fetch: function( id ){
			...
			// vars
			var data = fields.prepare_for_ajax({
				action		: 'fields/fields/gallery/get_attachment',
				field_key	: fields.get_field_key(this.$field),
		...
		render_fetch: function( html ){
			...
			// vars
			var $side = this.$el.find('.fields-gallery-side-data');
			...
			// remove fields form data
			$side.find('.compat-field-fields-form-data').remove();
		...
		update_attachment: function(){
			...
			var $a = this.$el.find('.update-attachment')
				$form = this.$el.find('.fields-gallery-side-data'),
				data = fields.serialize_form( $form );
			...
			$a.before('<i class="fields-loading"></i>');
			
			
			// append AJAX action		
			data.action = 'fields/fields/gallery/update_attachment';
			
			
			// prepare for ajax
			fields.prepare_for_ajax(data);
		...
		add: function( a ){
			...
			if( this.o.max > 0 && this.count() >= this.o.max ) {
				...
				fields.validation.add_warning( this.$field, fields._e('gallery', 'max'));
			...
			'<div class="' + thumb_class + '" data-id="' + a.id + '">',
				...
				'<div class="actions fields-soh-target">',
					'<a href="#" class="fields-icon fields-icon-cancel dark remove-attachment" data-id="' + a.id + '"></a>',
				'</div>',
			'</div>'].join('');
			
			
			// append
			this.$el.find('.fields-gallery-attachments').append( html );
		...
		edit_attachment:function( e ){
			...
			// vars
			var id = fields.get_data(e.$el, 'id');
			...
			var frame = fields.media.popup({
				...
				select:		function( attachment ){
					...
					// override url
					if( fields.isset(attachment, 'attributes', 'sizes', self.o.preview_size, 'url') ) {
		...
		add_attachment: function( e ){
			...
			var frame = fields.media.popup({
				...
				title:		fields._e('gallery', 'select'),
				mode:		'select',
				type:		'',
				field:		fields.get_field_key(this.$field),
				multiple:	'add',
				...
				select: function( attachment, i ) {
			    	...
			    	if( a.type === 'image' ) {
				    	...
				    	a.url = fields.maybe_get(atts, 'sizes.'+preview_size+'.url', atts.url);
				    	
			    	} else {
				    	
				    	a.url = fields.maybe_get(atts, 'thumb.src', '');
...
// @codekit-prepend "../js/fields-pro.js";
// @codekit-prepend "../js/fields-repeater.js";
// @codekit-prepend "../js/fields-flexible-content.js";
// @codekit-prepend "../js/fields-gallery.js";
