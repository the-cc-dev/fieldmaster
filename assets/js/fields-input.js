...
(function($){
	...
	*/
	...
	fields = {
		...
		*  update
		*
		*  This function will update a value found in fields.o
		*
		*  @type	function
		*  @date	8/09/2014
		*  @since	5.0.0
		...
		*  get
		*
		*  This function will return a value found in fields.o
		*
		*  @type	function
		*  @date	8/09/2014
		*  @since	5.0.0
		...
		*  _e
		*
		*  This functiln will return a string found in fields.l10n
		*
		*  @type	function
		*  @date	8/09/2014
		*  @since	5.0.0
		...
		*/
		...
		remove_action: function() {
			...
			// prefix action
			arguments[0] = 'fields.' + arguments[0];
		...
		*/
		...
		get_fields: function( s, $el, all ){
			...
			// debug
			//console.log( 'fields.get_fields(%o, %o, %o)', args, $el, all );
			//console.time("fields.get_fields");
			...
			if( !all ) {
				...
				$fields = fields.apply_filters('get_fields', $fields);
								
			}
			
			
			//console.log('get_fields(%o, %o, %o) %o', s, $el, all, $fields);
			//console.log('fields.get_fields(%o):', this.get_selector(s) );
			//console.timeEnd("fields.get_fields");
		...
		*/
		...
		serialize_form : function( $el, prefix ){
			...
			$.each( $selector.serializeArray(), function( i, pair ) {
				...
				// bail early if name does not start with fields or _fields
		...
		*/
		...
		remove_tr : function( $tr, callback ){
			...
			// add class
			$tr.addClass('fields-remove-element');
			...
			setTimeout(function(){
				...
				// remove class
				$tr.removeClass('fields-remove-element');
		...
		*/
		...
		remove_el : function( $el, callback, end_height ){
			...
			// wrap field
			$el.wrap( '<div class="fields-temp-wrap" style="height:' + $el.outerHeight(true) + 'px"></div>' );
			...
			// remove
			$el.parent('.fields-temp-wrap').animate({ height : end_height }, 250, function(){
		...
		*/
		...
		open_popup : function( args ){
			...
			var tmpl = [
				'<div id="fields-popup">',
					'<div class="fields-popup-box fields-box">',
						'<div class="title"><h3></h3><a href="#" class="fields-icon fields-icon-cancel grey fields-close-popup"></a></div>',
						'<div class="inner"></div>',
						'<div class="loading"><i class="fields-loading"></i></div>',
					'</div>',
					'<div class="bg"></div>',
				'</div>'
			].join('');
			...
			$('#fields-popup').on('click', '.bg, .fields-close-popup', function( e ){
		...
		*/
		...
		update_popup : function( args ){
			...
			if( args.width ) {
				...
				$popup.find('.fields-popup-box').css({
					'width'			: args.width,
		...
		*/
		...
		update_user_setting : function( name, value ) {
			...
			// ajax
			$.ajax({
		    	url			: fields.get('ajaxurl'),
				dataType	: 'html',
				type		: 'post',
				data		: fields.prepare_for_ajax({
					'action'	: 'fields/update_user_setting',
					'name'		: name,
					'value'		: value
				})
			});
		...
		*/
		...
		prepare_for_ajax : function( args ) {
			...
			// nonce
			args.nonce = fields.get('nonce');
			
			
			// filter for 3rd party customization
			args = fields.apply_filters('prepare_for_ajax', args);	
...
*/
		...
		*/
		...
		duplicate: function( $el, attr ){
			...
			// vars
			find = $el.attr(attr);
			replace = fields.get_uniqid();
			
			
			// allow fields to modify DOM
			// fixes bug where select field option is not selected
			fields.do_action('before_duplicate', $el);
			...
			// allow fields to modify DOM
			fields.do_action('after_duplicate', $el, $el2 );
	...
	*/
	...
	fields.model = {
		...
		_add_action: function( name, callback ) {
			...
			// add action
			fields.add_action(name, model[ callback ], priority, model);
		...
		_add_filter: function( name, callback ) {
			...
			// add action
			fields.add_filter(name, model[ callback ], priority, model);
	...
	*/
	...
	fields.layout = fields.model.extend({
		...
		refresh: function( $el ){
			...
			//console.time('fields.width.render');
			...
			$('.fields-fields:visible', $el).each(function(){
				...
				// get fields
				var $fields = $(this).children('.fields-field[data-width]:visible');
				...
				// reset fields
				$fields.removeClass('fields-r0 fields-c0').css({'min-height': 0});
				...
				$fields.each(function( i ){
					...
					if( this_top == 0 ) {
						...
						$el.addClass('fields-r0');
						
					} else if( cell == 0 ) {
						
						$el.addClass('fields-c0');
			...
			//console.timeEnd('fields.width.render');
	...
	*/
	...
	fields.field = {
		...
		extend: function( args ){
			...
			// vars
			var selector = fields.get_selector(model.type);
			...
			$.each(model.actions, function( k, callback ){
				...
				fields.add_action(action, function( $el ){
			...
			$.each(model.events, function( k, callback ){
				...
				$(document).on(event, selector + ' ' + trigger, function( e ){
					...
					// focus
					model.doFocus( fields.get_closest_field( $(this), model.type ) );
	...
	fields.add_action('prepare', function( $el ){
		
		fields.get_fields('', $el).each(function(){
			
			fields.do_action('prepare_field', $(this));
			fields.do_action('prepare_field/type=' + fields.get_field_type($(this)), $(this));
			
		});
		
	});
	
	fields.add_action('ready', function( $el ){
		
		fields.get_fields('', $el).each(function(){
			
			fields.do_action('ready_field', $(this));
			fields.do_action('ready_field/type=' + fields.get_field_type($(this)), $(this));
			
		});
		
	});
	
	fields.add_action('append', function( $el ){
				
		fields.get_fields('', $el).each(function(){
			
			fields.do_action('append_field', $(this));
			fields.do_action('append_field/type=' + fields.get_field_type($(this)), $(this));
			
		});
		
	});
	
	fields.add_action('load', function( $el ){
				
		fields.get_fields('', $el).each(function(){
			
			fields.do_action('load_field', $(this));
			fields.do_action('load_field/type=' + fields.get_field_type($(this)), $(this));
	...
	fields.add_action('remove', function( $el ){
				
		fields.get_fields('', $el).each(function(){
			
			fields.do_action('remove_field', $(this));
			fields.do_action('remove_field/type=' + fields.get_field_type($(this)), $(this));
			
		});
		
	});
	
	fields.add_action('sortstart', function( $item, $placeholder ){
				
		fields.get_fields('', $item).each(function(){
			
			fields.do_action('sortstart_field', $(this));
			fields.do_action('sortstart_field/type=' + fields.get_field_type($(this)), $(this));
			
		});
		
	});
	
	fields.add_action('sortstop', function( $item, $placeholder ){
				
		fields.get_fields('', $item).each(function(){
			
			fields.do_action('sortstop_field', $(this));
			fields.do_action('sortstop_field/type=' + fields.get_field_type($(this)), $(this));
			
		});
		
	});
	
	fields.add_action('hide_field', function( $el, context ){
				
		fields.do_action('hide_field/type=' + fields.get_field_type($el), $el, context);
		
	});
	
	fields.add_action('show_field', function( $el, context ){
				
		fields.do_action('show_field/type=' + fields.get_field_type($el), $el, context);
	...
	*/
	...
	$(document).on('change', '.fields-field input, .fields-field textarea, .fields-field select', function(){
		
		// preview hack
		if( $('#fields-form-data input[name="_fieldschanged"]').exists() ) {
		
			$('#fields-form-data input[name="_fieldschanged"]').val(1);
			
		}
		
		
		// action for 3rd party customization
		fields.do_action('change', $(this));
	...
	*/
	...
	$(document).on('click', '.fields-field a[href="#"]', function( e ){
	...
	*/
	...
	fields.unload = fields.model.extend({
	...
	fields.tooltip = fields.model.extend({
		
		$el: null,
		
		events: {
			'mouseenter .fields-js-tooltip':	'on',
			'mouseleave .fields-js-tooltip':	'off',
		},
		...
		on: function( e ){
			...
			// $t
			this.$el = $('<div class="fields-tooltip">' + title + '</div>');
	...
	fields.postbox = fields.model.extend({
		
		events: {
			'mouseenter .fields-postbox .handlediv':	'on',
			'mouseleave .fields-postbox .handlediv':	'off',
		},
		...
		render: function( args ){
			...
			// add class
			$postbox.addClass('fields-postbox');
			$label.addClass('fields-postbox-toggle');
			...
			// .inside class
			$postbox.children('.inside').addClass('fields-fields').addClass('-' + args.label);
			...
			if( args.visibility ) {
				...
				$postbox.addClass('fields-hidden');
				$label.addClass('fields-hidden');
			...
			if( args.edit_url ) {
				...
				$postbox.children('.hndle').append('<a href="' + args.edit_url + '" class="dashicons dashicons-admin-generic fields-hndle-cog fields-js-tooltip" title="' + args.edit_title + '"></a>');
	...
	*/
	...
	fields.add_action('before_duplicate', function( $orig ){
	...
	fields.add_action('after_duplicate', function( $orig, $duplicate ){
	...
	/*
console.time("fields_test_ready");
	console.time("fields_test_load");
	
	fields.add_action('ready', function(){
		
		console.timeEnd("fields_test_ready");
		
	}, 999);
	
	fields.add_action('load', function(){
		
		console.timeEnd("fields_test_load");
...
(function($){
	...
	fields.ajax = fields.model.extend({
		...
		events: {
			...
			'change .fields-taxonomy-field[data-save="1"] input':		'_change_term',
			'change .fields-taxonomy-field[data-save="1"] select':		'_change_term'
		},
		
		o: {
			'action':			'fields/post/get_field_groups',
			'post_id':			0,
			'page_template':	0,
			'page_parent':		0,
			'page_type':		0,
		...
		ready: function(){
			...
			// update post_id
			this.update('post_id', fields.get('post_id'));
		...
		fetch: function(){
			...
			$.ajax({
				url:		fields.get('ajaxurl'),
				data:		fields.prepare_for_ajax( this.o ),
				type:		'post',
				dataType:	'json',
				
				success: function( json ){
					
					if( fields.is_ajax_success( json ) ) {
		...
		render: function( json ){
			...
			// hide
			$('.fields-postbox').addClass('fields-hidden');
			$('.fields-postbox-toggle').addClass('fields-hidden');
			...
			$.each(json, function( k, field_group ){
				...
				// vars
				var $postbox = $('#fields-' + field_group.key),
					$toggle = $('#fields-' + field_group.key + '-hide'),
					$label = $toggle.parent();
					
				
				// show
				// use show() to force display when postbox has been hidden by 'Show on screen' toggle
				$postbox.removeClass('fields-hidden hide-if-js').show();
				$label.removeClass('fields-hidden hide-if-js').show();
				$toggle.prop('checked', true);
				
				
				// replace HTML if needed
				var $replace = $postbox.find('.fields-replace-with-fields');
				
				if( $replace.exists() ) {
					
					$replace.replaceWith( field_group.html );
					
					fields.do_action('append', $postbox);
				...
				if( k === 0 ) {
					...
					$('#fields-style').html( field_group.style );
					
				}
				
				
				// enable inputs
				$postbox.find('.fields-hidden-by-postbox').prop('disabled', false);
				
			});
			
			
			// disable inputs
			$('.fields-postbox.fields-hidden').find('select, textarea, input').not(':disabled').each(function(){
				
				$(this).addClass('fields-hidden-by-postbox').prop('disabled', true);
		...
		sync_taxonomy_terms: function(){
			...
			$('.categorychecklist, .fields-taxonomy-field').each(function(){
				...
				if( $el.is('.fields-taxonomy-field') && $el.attr('data-save') != '1' ) {
...
(function($){
	...
	fields.fields.checkbox = fields.field.extend({
		...
		change: function( e ){
			...
			// is toggle?
			if( e.$el.hasClass('fields-checkbox-toggle') ) {
			...
			if( !$ul.find('.fields-checkbox-toggle').exists() ) {
			...
			// update toggle
			$ul.find('.fields-checkbox-toggle').prop('checked', checked);
...
(function($){
	...
	fields.fields.color_picker = fields.field.extend({
...
(function($){
	...
	fields.conditional_logic = fields.model.extend({
		...
		events: {
			'change .fields-field input': 		'change',
			'change .fields-field textarea': 	'change',
			'change .fields-field select': 	'change'
		},
		...
		*/
		...
		render: function( $el ){
			...
			// get targets
			var $targets = fields.get_fields( '', $el, true );
		...
		*/
		...
		change: function( e ){
			...
			// vars
			var $input = e.$el,
				$field = fields.get_field_wrap( $input ),
				key = $field.data('key');
			...
			for( var i in this.triggers[ key ] ) {
				...
				// get targets
				var $targets = fields.get_fields(target_key, $parent, true);
			...
			// action for 3rd party customization
			fields.do_action('refresh', $parent);
		...
		*/
		...
		show_field: function( $field ){
			...
			*/
			...
			// remove "disabled"
			// ignore inputs which have a class of 'fields-disabled'. These inputs are disabled for life
			// ignore inputs which are hidden by conditiona logic of a sub field
			$field.find('.fields-clhi').not('.hidden-by-conditional-logic .fields-clhi').removeAttr('disabled');
			
			
			// action for 3rd party customization
			fields.do_action('show_field', $field, 'conditional_logic' );
		...
		*/
		...
		hide_field : function( $field ){
			...
			*/
			...
			// add "disabled"
			$field.find('input, textarea, select').not('.fields-disabled').addClass('fields-clhi').attr('disabled', 'disabled');
						
			
			// action for 3rd party customization
			fields.do_action('hide_field', $field, 'conditional_logic' );
		...
		*/
		...
		get_trigger: function( $target, key ){
			...
			// vars
			var selector = fields.get_selector( key );
			...
			if( !$trigger.exists() ) {
				...
				// vars
				var parent = fields.get_selector();
			...
			if( type == 'true_false' || type == 'checkbox' || type == 'radio' ) {
				...
				if( data.multiple && data.ui ) {
					...
					if( $select2.exists() ) {
						...
						$select2.find('.fields-select2-multi-choice').each(function(){
...
(function($){
	...
	fields.fields.date_picker = fields.field.extend({
		...
		focus: function(){
			...
			// get elements
			this.$el = this.$field.find('.fields-date_picker');
			this.$input = this.$el.find('input[type="text"]');
			this.$hidden = this.$el.find('input[type="hidden"]');
			
			// get options
			this.o = fields.get_data( this.$el );
		...
		initialize: function(){
			...
			// create options
			var args = $.extend( {}, fields.l10n.date_picker, { 
				dateFormat		:	'yymmdd',
			...
			// filter for 3rd party customization
			args = fields.apply_filters('date_picker_args', args, this.$field);
			...
			if( $('body > #ui-datepicker-div').exists() ) {
				...
				$('body > #ui-datepicker-div').wrap('<div class="fields-ui-datepicker" />');
...
(function($){
	...
	fields.fields.file = fields.field.extend({
		...
		focus: function(){
			...
			// get elements
			this.$el = this.$field.find('.fields-file-uploader');
		...
		add : function() {
			...
			// get repeater
			var $repeater = fields.get_closest_field( $field, 'repeater' );
			
			
			// popup
			var frame = fields.media.popup({
				
				title:		fields._e('file', 'select'),
				mode:		'select',
				type:		'',
				field:		fields.get_field_key($field),
				...
				select: function( attachment, i ) {
			    	...
			    	if( i > 0 ) {
						...
						// vars
						var key = fields.get_field_key( $field ),
							$tr = $field.closest('.fields-row');
						...
						$tr.nextAll('.fields-row:visible').each(function(){
							
							// get next $field
							$field = fields.get_field( key, $(this) );
							...
							if( $field.find('.fields-file-uploader.has-value').exists() ) {
						...
						if( !$field ) {
							...
							$tr = fields.fields.repeater.doFocus( $repeater ).add();
		...
		edit : function() {
			...
			var frame = fields.media.popup({
				...
				title:		fields._e('file', 'edit'),
				button:		fields._e('file', 'update'),
				mode:		'edit',
				id:			id,
...
(function($){
	...
	fields.fields.google_map = fields.field.extend({
		...
		events: {
			...
			'blur .search': 					'_blur',
			'paste .search': 					'_paste',
			'mousedown .fields-google-map':		'_mousedown',
		},
		
		focus: function(){
			
			// get elements
			this.$el = this.$field.find('.fields-google-map');
		...
		*/
		...
		initialize: function(){
			...
			// map
			var map_args = fields.apply_filters('google_map_args', {
			...
			// marker
			var marker_args = fields.apply_filters('google_map_marker_args', {
		...
		update: function( lat, lng ){
		    ...
		    // update inputs
		    fields.val( this.$el.find('.input-lat'), lat );
		    fields.val( this.$el.find('.input-lng'), lng );
			...
			// action
			fields.do_action('google_map_change', latlng, this.map, this.$field);
		...
		sync: function(){
			...
			this.geocoder.geocode({ 'latLng' : latlng }, function( results, status ){
				...
				// update input
				fields.val( self.$el.find('.input-address'), location.formatted_address );
		...
		*/
		...
		_clear: function( e ){ // console.log('_clear');
			...
			// clear inputs
			fields.val( this.$el.find('.input-address'), '' );
			fields.val( this.$el.find('.input-lat'), '' );
			fields.val( this.$el.find('.input-lng'), '' );
		...
		_locate: function( e ){ // console.log('_locate');
			...
			if( !navigator.geolocation ) {
				...
				alert( fields._e('google_map', 'browser_support') );
				return this;
...
(function($){
	...
	fields.fields.image = fields.field.extend({
		...
		focus: function(){
			...
			// get elements
			this.$el = this.$field.find('.fields-image-uploader');
		...
		add: function() {
			...
			// get repeater
			var $repeater = fields.get_closest_field( this.$field, 'repeater' );
			
			
			// popup
			var frame = fields.media.popup({
				
				title:		fields._e('image', 'select'),
				mode:		'select',
				type:		'image',
				field:		fields.get_field_key($field),
				multiple:	$repeater.exists(),
				...
				select: function( attachment, i ) {
			    	...
			    	if( i > 0 ) {
						...
						$tr.nextAll('.fields-row:visible').each(function(){
							...
							// bail early if next file uploader has value
							if( $field.find('.fields-image-uploader.has-value').exists() ) {
		...
		prepare: function( attachment ) {
			...
			if( fields.isset(attachment.attributes, 'sizes', this.o.preview_size, 'url') ) {
		...
		edit: function() {
			...
			var frame = fields.media.popup({
				...
				title:		fields._e('image', 'edit'),
				button:		fields._e('image', 'update'),
				mode:		'edit',
				id:			id,
...
(function($){
	...
	fields.media = fields.model.extend({
		...
		popup: function( args ) {
			...
			// vars
			var post_id = fields.get('post_id');
			...
			// add _fieldsuploader
			// this is super wack!
			// if you add _fieldsuploader to the options.library args, new uploads will not be added to the library view.
			// this has been traced back to the wp.media.model.Query initialize function (which can't be overriden)
			// Adding any custom args will cause the Attahcments to not observe the uploader queue
			// To bypass this security issue, we add in the args AFTER the Query has been initialized
			// options.library._fieldsuploader = args.field;
			if( fields.isset(Query, 'mirroring', 'args') ) {
				
				Query.mirroring.args._fieldsuploader = args.field;
			...
			if( fields.isset(wp, 'media', 'controller', 'EditImage') ) {
			...
			// plupload
			if( fields.isset(_wpPluploadSettings, 'defaults', 'multipart_params') ) {
				
				// add _fieldsuploader so that Uploader will inherit
...
*/
			...
			frame.on('content:activate:browse', function(){
				...
				if( args.type == 'image' ) {
					...
					// update all
					filters.filters.all.text = fields._e('image', 'all');
				...
				if( args.library == 'uploadedTo' ) {
					...
					// add 'uploadedTo' text
					filters.$el.parent().append('<span class="fields-uploadedTo">' + fields._e('image', 'uploadedTo') + '</span>');
				...
				// add _fieldsuploader to filters
				$.each( filters.filters, function( k, filter ){
					
					filter.props._fieldsuploader = args.field;
			...
			frame.on('open',function() {
				...
				// add class
				this.$el.closest('.media-modal').addClass('fields-media-modal fields-expanded');
			...
			frame.on('close',function(){
				...
				// remove class
				frame.$el.closest('.media-modal').removeClass('fields-media-modal');
		...
		ready: function(){
			...
			// vars
			var version = fields.get('wp_version'),
				post_id = fields.get('post_id');
			
			
			// update wp.media
			if( fields.isset(window,'wp','media','view','settings','post') && $.isNumeric(post_id) ) {
			...
			if( version ) {
				...
				// add body class
				$('body').addClass('fields-wp-' + version);
				
			}
			
			
			// customize wp.media views
			if( fields.isset(window, 'wp', 'media', 'view') ) {
		...
		customize_Attachment: function(){
			...
			wp.media.view.Attachment.Library = AttachmentLibrary.extend({
				...
				render: function() {
					...
					// vars
					var frame = fields.media.frame(),
						errors = fields.maybe_get(this, 'model.attributes.fields_errors');
					...
					if( frame && errors ) {
						...
						this.$el.addClass('fields-disabled');
				...
				toggleSelection: function( options ) {
					...
					$sidebar.children('.fields-selection-error').remove();
					
					
					// show attachment details
					$sidebar.children().removeClass('fields-hidden');
					...
					if( frame && errors ) {
						...
						// vars
						var filename = fields.maybe_get(this, 'model.attributes.filename', '');
						
						
						// hide attachment details
						// Gallery field continues to show previously selected attachment...
						$sidebar.children().addClass('fields-hidden');
						
						
						// append message
						$sidebar.prepend([
							'<div class="fields-selection-error">',
								'<span class="selection-error-label">' + fields._e('restricted') +'</span>',
		...
		customize_AttachmentCompat: function(){
			...
			view.render = function() {
				...
				setTimeout(function(){
					...
					if( $media_model.hasClass('fields-media-modal') ) {
					...
					if( $media_model.find('.media-frame-router .fields-expand-details').exists() ) {
					...
					// create button
					var $a = $([
						'<a href="#" class="fields-expand-details">',
							'<span class="is-closed"><span class="fields-icon fields-icon-left small grey"></span>' + fields._e('expand_details') +  '</span>',
							'<span class="is-open"><span class="fields-icon fields-icon-right small grey"></span>' + fields._e('collapse_details') +  '</span>',
						'</a>'
					].join('')); 
					...
					$a.on('click', function( e ){
						...
						if( $media_model.hasClass('fields-expanded') ) {
						
							$media_model.removeClass('fields-expanded');
							
						} else {
							
							$media_model.addClass('fields-expanded');
				...
				clearTimeout( fields.media.render_timout );
				fields.media.render_timout = setTimeout(function(){
					
					fields.do_action('append', self.$el);
			...
			view.dispose = function() {
				...
				// remove
				fields.do_action('remove', this.$el);
			...
			view.save = function( e ) {
				...
				// serialize form
				var data = fields.serialize_form(this.$el);
...
(function($){
	...
	fields.fields.oembed = {
		...
		search : function( $el ){ 
			...
			// AJAX data
			var ajax_data = {
				'action'	: 'fields/fields/oembed/search',
				'nonce'		: fields.get('nonce'),
				's'			: s,
				'width'		: fields.get_data($el, 'width'),
				'height'	: fields.get_data($el, 'height')
			};
			...
			var xhr = $.ajax({
				...
				success: function( html ){
					...
					// update from json
					fields.fields.oembed.search_success( $el, s, html );
					
					
					// no results?
					if( !html )
					{
						fields.fields.oembed.search_error( $el );
					}
	...
	*/
	...
	$(document).on('click', '.fields-oembed [data-name="search-button"]', function( e ){
		
		e.preventDefault();
		
		fields.fields.oembed.search( $(this).closest('.fields-oembed') );
		
		$(this).blur();
		
	});
	
	$(document).on('click', '.fields-oembed [data-name="clear-button"]', function( e ){
		
		e.preventDefault();
		
		fields.fields.oembed.clear( $(this).closest('.fields-oembed') );
		
		$(this).blur();
		
	});
	
	$(document).on('click', '.fields-oembed [data-name="value-title"]', function( e ){
		
		e.preventDefault();
		
		fields.fields.oembed.edit( $(this).closest('.fields-oembed') );
			
	});
	
	$(document).on('keypress', '.fields-oembed [data-name="search-input"]', function( e ){
	...
	$(document).on('keyup', '.fields-oembed [data-name="search-input"]', function( e ){
	...
	$(document).on('blur', '.fields-oembed [data-name="search-input"]', function(e){
		
		fields.fields.oembed.blur( $(this).closest('.fields-oembed') );
...
(function($){
	...
	fields.fields.radio = fields.field.extend({
...
(function($){
	...
	fields.fields.relationship = fields.field.extend({
		...
		events: {
			...
			'click .choices .fields-rel-item': 	'add_item',
		...
		focus: function(){
			...
			// get elements
			this.$el = this.$field.find('.fields-relationship');
			this.$input = this.$el.find('.fields-hidden input');
...
/*
		show: function(){
			...
			// is field in view
		    if( fields.is_in_view(this.$field) ) {
...
*/
		...
		fetch: function(){
			...
			// add to this.o
			this.o.action = 'fields/fields/relationship/query';
			this.o.field_key = $field.data('key');
			this.o.post_id = fields.get('post_id');
			
			
			// ready for ajax
			var ajax_data = fields.prepare_for_ajax( this.o );
			...
			// add message
			this.$choices.find('ul:last').append('<p><i class="fields-loading"></i> ' + fields._e('relationship', 'loading') + '</p>');
		...
		render: function( json ){
			...
			if( !json || !json.length ) {
				...
				if( this.o.paged == 1 ) {
					...
					this.$choices.children('.list').append('<p>' + fields._e('relationship', 'empty') + '</p>');
			...
			this.$values.find('.fields-rel-item').each(function(){
				
				$new.find('.fields-rel-item[data-id="' +  $(this).data('id') + '"]').addClass('disabled');
			...
			if( this.o.s ) {
				...
				$new.find('.fields-rel-item').each(function(){
			...
			this.$choices.find('.fields-rel-label').each(function(){
		...
		walker: function( data ){
			...
			if( $.isArray(data) ) {
				...
				if( data.children !== undefined ) {
					...
					s += '<li><span class="fields-rel-label">' + data.text + '</span><ul class="fields-bl">';
					...
					s += '<li><span class="fields-rel-item" data-id="' + data.id + '">' + data.text + '</span></li>';
		...
		add_item: function( e ){
			...
			if( this.o.max > 0 ) {
				...
				if( this.$values.find('.fields-rel-item').length >= this.o.max ) {
				
					alert( fields._e('relationship', 'max').replace('{max}', this.o.max) );
			...
			var html = [
				'<li>',
					...
					'<span data-id="' + e.$el.data('id') + '" class="fields-rel-item">' + e.$el.html(),
			...
			// validation
			fields.validation.remove_error( this.$field );
		...
		remove_item : function( e ){
			...
			// show
			this.$choices.find('.fields-rel-item[data-id="' + id + '"]').removeClass('disabled');
...
(function($){
	...
	fields.add_select2 = function( $select, args ) {
		...
		var select2_args = {
			width:				'100%',
			containerCssClass:	'fields-select2-container',
		...
		if( args.multiple ) {
			...
			select2_args.formatSelection = function( object, $div ){
				...
				$div.parent().append('<input type="hidden" class="fields-select2-multi-choice" name="' + $select.attr('name') + '" value="' + object.id + '" />');
		...
		if( value !== '' ) {
			...
			if( !args.multiple ) {
				...
				selected = fields.maybe_get(selected, 0, '');
		...
		if( args.ajax ) {
			...
			select2_args.ajax = {
				...
				data: function (term, page) {
					...
					// vars
					var data = fields.prepare_for_ajax({
						action: 	args.action,
						field_key: 	args.key,
						post_id: 	fields.get('post_id'),
						s: 			term,
		...
		// filter for 3rd party customization
		select2_args = fields.apply_filters( 'select2_args', select2_args, $select, args );
		...
		$select.attr('disabled', 'disabled').addClass('fields-disabled');

	}
	
	fields.remove_select2 = function( $select ) {
	...
	// select
	fields.fields.select = fields.field.extend({
		...
		focus: function(){
			...
			// get options
			this.o = fields.get_data( this.$select );
			
			
			// customize o
			this.o.pagination = this.pagination;
			this.o.key = this.$field.data('key');	
			this.o.action = 'fields/fields/' + this.type + '/query';
		...
		render: function(){
			...
			fields.add_select2( this.$select, this.o );
		...
		remove: function(){
			...
			fields.remove_select2( this.$select );
	...
	// user
	fields.fields.user = fields.fields.select.extend({
	...
	// post_object
	fields.fields.post_object = fields.fields.select.extend({
	...
	// page_link
	fields.fields.page_link = fields.fields.select.extend({
...
(function($){
	...
	fields.fields.tab = fields.field.extend({
		...
		focus: function(){
			...
			// get elements
			this.$el = this.$field.find('.fields-tab');
		...
		initialize: function(){
			...
			if( !this.$field.siblings('.fields-tab-wrap').exists() ) {
		...
		add_tab: function(){
			...
			// vars
			var $group = this.$field.siblings('.fields-tab-wrap').last();
			
			
			// vars
			var $li = $([
				'<li>',
					'<a class="fields-tab-button" href="#" data-key="' + this.$field.data('key') + '">' + this.$el.text() + '</a>',
				'</li>'
			].join(''));
		...
		add_group : function(){
			...
			// add sidebar to wrap
			if( $wrap.hasClass('fields-fields') && placement == 'left' ) {
...
*/
			...
			if( $wrap.is('tbody') ) {
				...
				html = '<tr class="fields-tab-wrap"><td colspan="2"><ul class="fields-hl fields-tab-group"></ul></td></tr>';
			
			} else {
			
				html = '<div class="fields-tab-wrap -' + placement + '"><ul class="fields-hl fields-tab-group"></ul></div>';
		...
		toggle : function( $a ){
			...
			// vars
			var $wrap = $a.closest('.fields-tab-wrap');
			...
			// loop over tab fields (.fields-field-tab) until you hit another group (.fields-tab-wrap)
			$wrap.nextUntil('.fields-tab-wrap', '.fields-field-tab').each(function(){
		...
		show_tab_fields : function( $field ) {
			...
			// loop over all fields (.fields-field) until you hit another tab field (.fields-field-tab)
			$field.nextUntil('.fields-field-tab', '.fields-field').each(function(){
				...
				// do action
				fields.do_action('show_field', $(this), 'tab');
		...
		hide_tab_fields : function( $field ) {
			...
			$field.nextUntil('.fields-field-tab', '.fields-field').each(function(){
				...
				// do action
				fields.do_action('hide_field', $(this), 'tab');
		...
		hide: function( $field, context ){
			...
			// vars
			var $a = $field.siblings('.fields-tab-wrap').find('a[data-key="' + $field.data('key') + '"]'),
				$li = $a.parent();
			...
			$field.nextUntil('.fields-field-tab', '.fields-field').each(function(){
				...
				fields.conditional_logic.hide_field( $(this) );
		...
		show: function( $field, context ){
			...
			$field.nextUntil('.fields-field-tab', '.fields-field').each(function(){
				...
				fields.conditional_logic.show_field( $(this) );
	...
	*/
	...
	$(document).on('click', '.fields-tab-button', function( e ){
		
		e.preventDefault();
		
		fields.fields.tab.toggle( $(this) );
		
		$(this).trigger('blur');
			
	});
	
	var tab_validation = fields.model.extend({
		...
		add_field_error: function( $field ){
			...
			// vars
			var $tab = $field.prevAll('.fields-field-tab:first'),
				$group = $field.prevAll('.fields-tab-wrap:first');
...
(function($){
	...
	// taxonomy
	fields.fields.taxonomy = fields.field.extend({
		...
		focus: function(){
			...
			// $el
			this.$el = this.$field.find('.fields-taxonomy-field');
		...
		render: function(){
			...
			// select2 options
			var args = fields.get_data( $select );
			
			
			// customize args
			args.pagination = true;
			args.key = this.o.key;	
			args.action = 'fields/fields/taxonomy/query';
			
			
			// add select2
			fields.add_select2( $select, args );
		...
		add_term: function( e ){
			...
			// AJAX data
			var ajax_data = fields.prepare_for_ajax({
				action:		'fields/fields/taxonomy/add_term',
		...
		add_term_confirm: function( html ){
			...
			// focus
			$('#fields-popup input[name="term_name"]').focus();
			
			
			// events
			$('#fields-popup form').on('submit', function( e ){
		...
		add_term_submit: function( $form ){
			...
			// vars
			var $submit = $form.find('.fields-submit'),
			...
			$submit.find('.fields-spinner').addClass('is-active');
			...
			$.ajax({
				...
				complete: function(){
					...
					// hide loading
					$submit.find('.fields-spinner').removeClass('is-active');
		...
		append_new_term: function( term ){
			...
			$('.fields-taxonomy-field[data-taxonomy="' + this.o.taxonomy + '"]').each(function(){
				...
				if( term.term_parent ) {
					...
					if( !$ul.exists() ) {
						...
						$ul = $('<ul class="children fields-bl"></ul>');
			...
			// append to select
			$('#fields-popup #term_parent').each(function(){
...
(function($){
	...
	fields.fields.url = fields.field.extend({
...
(function($){
	...
	fields.validation = fields.model.extend({
		...
		events: {
			...
			'click .fields-error-message a':	'click_message'
		},
		...
		// classes
		error_class: 'fields-error',
		message_class: 'fields-error-message',
		...
		*/
		...
		ready: function( $el ){
			...
			// reference
			$el.find('.fields-field input').filter('[type="number"], [type="email"], [type="url"]').on('invalid', function( e ){
				...
				// run validation
				fields.validation.fetch( $(this).closest('form') );
		...
		*/
		...
		toggle: function( $form, state ){
			...
			if( !$parent.exists() ) {
				...
				$parent = $form.find('.fields-form-submit');
			...
			$spinner = $parent.find('.spinner, .fields-spinner');
		...
		*/
		...
		fetch: function( $form ){
			...
			// vars
			var data = fields.serialize_form( $form, 'fields' );
				
			
			// append AJAX action		
			data.action = 'fields/validate_save_post';
		...
		*/
		...
		fetch_complete: function( $form ){
			...
			var $message = $form.children('.fields-error-message');
			
			if( $message.exists() ) {
				
				$message.removeClass('error');
				$message.children('p').html( fields._e('validation_successful') );
				
			}
			
		
			// remove hidden postboxes (this will stop them from being posted to save)
			$form.find('.fields-postbox.fields-hidden').remove();
			
			
			// action for 3rd party customization
			fields.do_action('submit', $form);
		...
		*/
		...
		fetch_success: function( $form, json ){
			...
			// filter for 3rd party customization
			json = fields.apply_filters('validation_complete', json, $form);
			...
			var $scrollTo = null,
				count = 0,
				message = fields._e('validation_failed');
			...
			if( json.errors && json.errors.length > 0 ) {
				...
				for( var i in json.errors ) {
					...
					// now get field
					var $field = fields.get_field_wrap( $input );
				...
				if( count == 1 ) {
					...
					message += '. ' + fields._e('validation_failed_1');
					
				} else if( count > 1 ) {
					
					message += '. ' + fields._e('validation_failed_2').replace('%d', count);
			...
			if( !$message.exists() ) {
				...
				$message = $('<div class="fields-error-message"><p></p><a href="#" class="fields-icon fields-icon-cancel small"></a></div>');
		...
		*/
		...
		add_error: function( $field, message ){
			...
			if( message !== undefined ) {
				...
				$field.children('.fields-input').children('.' + this.message_class).remove();
				$field.children('.fields-input').prepend('<div class="' + this.message_class + '"><p>' + message + '</p></div>');
			...
			fields.do_action('add_field_error', $field);
		...
		*/
		...
		remove_error: function( $field ){
			...
			// var
			$message = $field.children('.fields-input').children('.' + this.message_class);
			...
			setTimeout(function(){
				...
				fields.remove_el( $message );
				
			}, 250);
			
			
			// hook for 3rd party customization
			fields.do_action('remove_field_error', $field);
		...
		*/
		...
		add_warning: function( $field, message ){
			...
			setTimeout(function(){
				...
				fields.validation.remove_error( $field )
		...
		*/
		...
		show_spinner: function( $spinner ){
			...
			// vars
			var wp_version = fields.get('wp_version');
...
(function($){
	...
	fields.fields.wysiwyg = fields.field.extend({
		...
		initialize: function(){
			...
			var old_id = this.o.id,
				new_id = fields.get_uniqid('fields-editor-'),
				html = this.$el.outerHTML();
			
			
			// replace
			html = fields.str_replace( old_id, new_id, html );
		...
		get_mceInit : function(){
			...
			var toolbar = this.get_toolbar( this.o.toolbar ),
				mceInit = $.extend({}, tinyMCEPreInit.mceInit.fields_content);
			...
			if( toolbar ) {
				...
				for( var i = 1; i < 5; i++ ) {
					...
					mceInit[ k + i ] = fields.isset(toolbar, i) ? toolbar[i] : '';
...
*/
			...
			// hook for 3rd party customization
			mceInit = fields.apply_filters('wysiwyg_tinymce_settings', mceInit, mceInit.id);
		...
		get_qtInit : function(){
			...
			// vars
			var qtInit = $.extend({}, tinyMCEPreInit.qtInit.fields_content);
			...
			// hook for 3rd party customization
			qtInit = fields.apply_filters('wysiwyg_quicktags_settings', qtInit, qtInit.id);
		...
		*/
		...
		enable: function(){
			...
			if( this.$el.hasClass('tmce-active') && fields.isset(window,'switchEditors') ) {
	...
	$(document).ready(function(){
		...
		// move fields_content wysiwyg
		if( $('#wp-fields_content-wrap').exists() ) {
			
			$('#wp-fields_content-wrap').parent().appendTo('body');
...
// @codekit-prepend "../js/fields.js";
// @codekit-prepend "../js/fields-ajax.js";
// @codekit-prepend "../js/fields-checkbox.js";
// @codekit-prepend "../js/fields-color-picker.js";
// @codekit-prepend "../js/fields-conditional-logic.js";
// @codekit-prepend "../js/fields-date-picker.js";
// @codekit-prepend "../js/fields-file.js";
// @codekit-prepend "../js/fields-google-map.js";
// @codekit-prepend "../js/fields-image.js";
// @codekit-prepend "../js/fields-media.js";
// @codekit-prepend "../js/fields-oembed.js";
// @codekit-prepend "../js/fields-radio.js";
// @codekit-prepend "../js/fields-relationship.js";
// @codekit-prepend "../js/fields-select.js";
// @codekit-prepend "../js/fields-tab.js";
// @codekit-prepend "../js/fields-taxonomy.js";
// @codekit-prepend "../js/fields-url.js";
// @codekit-prepend "../js/fields-validation.js";
// @codekit-prepend "../js/fields-wysiwyg.js";
