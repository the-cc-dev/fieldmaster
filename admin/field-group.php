...
if( ! class_exists('fields_admin_field_group') ) :
...
class fields_admin_field_group {
	...
	*/
	...
	function __construct() {
		...
		// ajax
		add_action('wp_ajax_fields/field_group/render_field_settings',		array($this, 'ajax_render_field_settings'));
		add_action('wp_ajax_fields/field_group/render_location_value',		array($this, 'ajax_render_location_value'));
		add_action('wp_ajax_fields/field_group/move_field',				array($this, 'ajax_move_field'));
	...
	*/
	...
	function post_updated_messages( $messages ) {
		...
		// append to messages
		$messages['fields-field-group'] = array(
	...
	*/
	...
	function current_screen() {
		...
		// validate screen
		if( !fields_is_screen('fields-field-group') ) {
	...
	*/
	...
	function admin_enqueue_scripts() {
		...
		// custom scripts
		wp_enqueue_style('fields-field-group');
		wp_enqueue_script('fields-field-group');


		// 3rd party hook
		do_action('fields/field_group/admin_enqueue_scripts');
	...
	*/
	...
	function admin_head() {
		...
		// vars
		$field_group = fields_get_field_group( $post );


		// vars
		$l10n = apply_filters('fields/field_group/admin_l10n', array(
			'move_to_trash'			=> __("Move to trash. Are you sure?",'fields'),
			'checked'				=> __("checked",'fields'),
			'no_fields'				=> __("No toggle fields available",'fields'),
			'title_is_required'		=> __("Field group title is required",'fields'),
			'copy'					=> __("copy",'fields'),
			'or'					=> __("or",'fields'),
			'fields'				=> __("Fields",'fields'),
			'parent_fields'			=> __("Parent fields",'fields'),
			'sibling_fields'		=> __("Sibling fields",'fields'),
			'move_field'			=> __("Move Custom Field",'fields'),
			'move_field_warning'	=> __("This field cannot be moved until its changes have been saved",'fields'),
			'null'					=> __("Null",'fields'),
			'unload'				=> __('The changes you made will be lost if you navigate away from this page','fields'),
			'field_name_start'		=> __('The string "field_" may not be used at the start of a field name','fields'),
		));
		...
		// metaboxes
		add_meta_box('fields-field-group-fields', __("Fields",'fields'), array($this, 'mb_fields'), 'fields-field-group', 'normal', 'high');
		add_meta_box('fields-field-group-locations', __("Location",'fields'), array($this, 'mb_locations'), 'fields-field-group', 'normal', 'high');
		add_meta_box('fields-field-group-options', __("Settings",'fields'), array($this, 'mb_options'), 'fields-field-group', 'normal', 'high');
		...
		do_action('fields/field_group/admin_head');
	...
	*/
	...
	function screen_settings( $html ) {
		...
		// vars
		$checked = fields_get_user_setting('show_field_keys') ? 'checked="checked"' : '';


		// append
	    $html .= '<div id="fields-append-show-on-screen" class="fields-hidden">';
	    $html .= '<label for="fields-field-key-hide"><input id="fields-field-key-hide" type="checkbox" value="1" name="show_field_keys" ' . $checked . ' /> ' . __('Field Keys','fields') . '</label>';
		$html .= '</div>';
	...
	*/
	...
	function post_submitbox_misc_actions() {
		...
		// vars
		$status = $field_group['active'] ? __("Active",'fields') : __("Disabled",'fields');
...
<?php
	...
	*/
	...
	function edit_form_after_title() {
		...
		<div id="fields-form-data" class="fields-hidden">
			<input type="hidden" name="_fieldsnonce" value="<?php echo wp_create_nonce( 'field_group' ); ?>" />
			<input type="hidden" name="_fields_delete_fields" value="0" id="input-delete-fields" />
			<?php do_action('fields/field_group/form_data'); ?>
		</div>
		<?php
	...
	*/
	...
	function save_post( $post_id, $post ) {
		...
		// bail early if not fields-field-group
		if( $post->post_type !== 'fields-field-group' ) {
		...
		// verify nonce
		if( !fields_verify_nonce('field_group') ) {
		...
		if( !empty($_POST['fieldmaster_fields']) ) {
			...
			foreach( $_POST['fieldmaster_fields'] as $field ) {
				...
				// vars
				$specific = false;
				$save = fields_extract_var( $field, 'save' );
				...
				// save field
				fields_update_field( $field, $specific );
		...
		// delete fields
        if( $_POST['_fields_delete_fields'] ) {

	    	$ids = explode('|', $_POST['_fields_delete_fields']);
	...
	*  mb_fields
	*
	*  This function will render the HTML for the medtabox 'fields-field-group-fields'
	*
	*  @type	function
	*  @date	28/09/13
	*  @since	5.0.0
	...
	*/
	...
	function mb_fields() {
		...
		// get fields
		$view = array(
			'fields' => fields_get_fields_by_id( $field_group['ID'] )
		);


		// load view
		fields_get_view('field-group-fields', $view);
	...
	*  mb_options
	*
	*  This function will render the HTML for the medtabox 'fields-field-group-options'
	*
	*  @type	function
	*  @date	28/09/13
	*  @since	5.0.0
	...
	*/
	...
	function mb_options() {
		...
		if( !fields_is_field_group_key( $field_group['key']) ) {
		...
		include( fields_get_path('admin/views/field-group-options.php') );
	...
	*  mb_locations
	*
	*  This function will render the HTML for the medtabox 'fields-field-group-locations'
	*
	*  @type	function
	*  @date	28/09/13
	*  @since	5.0.0
	...
	*/
	...
	function mb_locations() {
		...
		// don't use view because we need access to $this context
		include( fields_get_path('admin/views/field-group-locations.php') );
	...
	*/
	...
	function render_location_value( $options ) {
		...
		switch( $options['param'] ) {
			...
			*/
			...
			case "post_type" :
				...
				$choices = fields_get_post_types( $exclude );
				$choices = fields_get_pretty_post_types( $choices );
			...
			case "post" :
				...
				$post_types = fields_get_post_types( $exclude );


				// get posts grouped by post type
				$groups = fields_get_grouped_posts(array(
				...
				if( !empty($groups) ) {
					...
					foreach( array_keys($groups) as $group_title ) {
						...
						foreach( array_keys($posts) as $post_id ) {
							...
							// update
							$posts[ $post_id ] = fields_get_post_title( $posts[ $post_id ] );
			...
			case "post_category" :
				...
				$terms = fields_get_taxonomy_terms( 'category' );
			...
			*/
			...
			case "page_template" :
				...
				$choices = array(
					'default'	=>	__("Default Template",'fields'),
				);
			...
			*/
			...
			case "current_user_role" :
				...
				if( is_multisite() ) {
					...
					$choices = array_merge(array(
						'super_admin' => __('Super Admin', 'fields')
					), $choices);
			...
			case "user_role" :
				...
				$choices = array_merge( array('all' => __('All', 'fields')), $wp_roles->get_names() );
			...
			*/
			...
			case "taxonomy" :
				...
				$choices = array_merge( array('all' => __('All', 'fields')), fields_get_taxonomies() );
		...
		$choices = apply_filters( 'fields/location/rule_values/' . $options['param'], $choices );
	...
	*/
	...
	function ajax_move_field() {
		...
		if( $args['field_id'] && $args['field_group_id'] ) {
			...
			// vars
			$field = fields_get_field($args['field_id']);
			$field_group = fields_get_field_group($args['field_group_id']);
			...
			echo '<p>' . sprintf( __('The %s field can now be found in the %s field group', 'fields'), $v1, $v2 ). '</p>';

			echo '<a href="#" class="fields-button blue fields-close-popup">' . __("Close Window",'fields') . '</a>';
		...
		echo '<form id="fields-move-field-form">';

			// render
			fields_render_field_wrap( $field );

			echo '<button type="submit" class="fields-button blue">' . __("Move Field",'fields') . '</button>';
