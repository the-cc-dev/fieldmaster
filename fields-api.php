...
Description: Fields API for create field interfaces and data storage in WordPress.
Version: 0.0.1
Author: GoldHat Group
Author URI: https://goldhat.ca
Copyright: GoldHat Group, Elliot Condon
Text Domain: fields-api
Domain Path: /lang
*/
...
class fieldsAPI {
	...
	*  __construct
	*
	*  A dummy constructor to ensure Fields API is only initialized once
	*
	*  @type	function
	*  @date	23/06/12
	*  @since	5.0.0
	...
	*  initialize
	*
	*  The real constructor to initialize Fields API
	*
	*  @type	function
	*  @date	28/09/13
	*  @since	5.0.0
	...
	*/
	...
	function wp_init() {
		...
		// complete loading of Fields API files
		$this->complete();
	...
	*/
	...
	function wp_posts_where( $where, $wp_query ) {
		...
		// fields_field_key
		if( $field_key = $wp_query->get('fields_field_key') ) {
	    ...
	    // fields_field_name
	    if( $field_name = $wp_query->get('fields_field_name') ) {
		...
		if( $field['type'] == 'repeater' ) {
			...
			if( !empty($layouts) ) {
				...
				foreach( array_keys($layouts) as $i ) {
					...
					// get valid layout (fixes Fields API4 export code bug undefined index 'key')
					if( empty($layout['key']) ) {
